...
export default class App extends Component {
  ...
  addToCart(item) {
    ...
    let index = this.state.cart.findIndex(items => items.id === item.id);
  ...
  deleteFromCart(id) {
    let cloneCart = this.state.cart;
    let index = this.state.cart.findIndex(items => items.id === id);
      ...
      <div className="App">
        <section className="products">
          ...
          {this.state.hats.map(item => (
            <div key={item.id} className={this.state.cardView?"product-A-view":"product-B-view"}>
        ...
        <section className="cart">
          ...
          <div className="inputs">
            ...
            <input
              ...
              onChange={(e) => this.handleCreditCardInput(e)}
            />
          </div>

          <button className="checkOut" onClick={() => this.checkout('Purchase is complete!')}>Checkout</button>
          <button className="cancel" onClick={() => this.checkout('Cart Cancelled!')}>Cancel Cart</button>
          <br/>
          
          {this.state.cart.map(item => (
            <div key={item.id} className="product-A-view">
              <img src={item.imageUrl} />
              <div className="product-info">
                <h4>{item.title}</h4>
